...
class FrEnDataset(Dataset):
    def __init__(self, pairs):
        # For French-to-English, we treat French as the source and English as the target.
        self.eng_vocab = Vocabulary()  # target: English
        self.fr_vocab = Vocabulary()   # source: French
        self.pairs = []
        for eng, fr in pairs:
            # Add French sentence to source vocab and English sentence to target vocab.
            self.fr_vocab.add_sentence(fr)
            self.eng_vocab.add_sentence(eng)
            self.pairs.append((eng, fr))
        self.src_sentences = [pair[1] for pair in self.pairs]  # French sentences
        self.tgt_sentences = [pair[0] for pair in self.pairs]  # English sentences
        self.src_tokens = tokenize_and_pad(self.src_sentences, self.fr_vocab)
        self.tgt_tokens = tokenize_and_pad(self.tgt_sentences, self.eng_vocab)
...
model = TransformerSeq2Seq(src_vocab_size=dataset.fr_vocab.n_words,
                           tgt_vocab_size=dataset.eng_vocab.n_words,
...
criterion = nn.CrossEntropyLoss(ignore_index=dataset.eng_vocab.word2index["<PAD>"])
...
def evaluate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for src, tgt in dataloader:
            ...
            non_pad = tgt_output != dataset.eng_vocab.word2index["<PAD>"]
...
def translate(model, sentence, dataset, device, max_length=20):
    model.eval()
    with torch.no_grad():
        tokens = [dataset.fr_vocab.word2index["<SOS>"]] + \
                 [dataset.fr_vocab.word2index[word] for word in sentence.split()] + \
                 [dataset.fr_vocab.word2index["<EOS>"]]
        src = torch.tensor(tokens, dtype=torch.long).unsqueeze(0).to(device)
        tgt_input = torch.tensor([[dataset.eng_vocab.word2index["<SOS>"]]], dtype=torch.long).to(device)
        for i in range(max_length):
            tgt_mask = model.generate_square_subsequent_mask(tgt_input.size(1)).to(device)
            output = model(src, tgt_input, tgt_mask=tgt_mask)
            next_token = output[:, -1, :].argmax(dim=-1).unsqueeze(0)
            tgt_input = torch.cat((tgt_input, next_token), dim=1)
            if next_token.item() == dataset.eng_vocab.word2index["<EOS>"]:
                break


        translation = []
        for idx in tgt_input.squeeze().tolist():
            if idx in [dataset.eng_vocab.word2index["<SOS>"], dataset.eng_vocab.word2index["<EOS>"], dataset.eng_vocab.word2index["<PAD>"]]:
                continue
            translation.append(dataset.eng_vocab.index2word[idx])
...
print("\n--- Qualitative Translations (French-to-English) ---")
...
for sentence in test_sentences:
    ...
    print(f"Generated English:{generated}\n")