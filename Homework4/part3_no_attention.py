...
encoder = Encoder(input_size=dataset.fr_vocab.n_words, hidden_size=256).to(device)
decoder = Decoder(hidden_size=256, output_size=dataset.eng_vocab.n_words).to(device)    
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=20):
    ...
    for di in range(target_length):
        ...
        if decoder_input.item() == dataset.eng_vocab.word2index['<EOS>']:  # Stop if EOS token is generated
            break
...
for epoch in range(n_epochs):
    total_loss = 0
    for fr_tokens, eng_tokens in dataloader:  # Ignore embeddings, we only need tokens
        eng_tokens = eng_tokens.squeeze(0).to(device)
        fr_tokens = fr_tokens.squeeze(0).to(device)

        loss = train(fr_tokens, eng_tokens, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion)
        total_loss += loss
...
def evaluate_and_show_examples(encoder, decoder, dataloader, dataset, criterion, n_examples=5):
    ...
    with torch.no_grad():
        for i, (input_tensor, target_tensor) in enumerate(dataloader):
            ...
            trimmed_target = [idx.item() for idx in target_tensor if idx.item() not in [SOS_token, EOS_token, dataset.eng_vocab.word2index["<PAD>"]]]
            trimmed_pred = [idx for idx in predicted_indices if idx not in [SOS_token, EOS_token, dataset.eng_vocab.word2index["<PAD>"]]]
            if trimmed_target == trimmed_pred:
                correct_predictions += 1

            if i < n_examples:
                input_words = [dataset.fr_vocab.index2word[idx.item()] for idx in input_tensor if idx.item() not in [SOS_token, EOS_token, dataset.fr_vocab.word2index["<PAD>"]]]
                target_words = [dataset.eng_vocab.index2word[idx] for idx in trimmed_target]
                predicted_words = [dataset.eng_vocab.index2word[idx] for idx in trimmed_pred]
